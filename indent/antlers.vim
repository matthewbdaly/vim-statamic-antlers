...
if exists('g:antlers_custom_directives_pairs')
    let s:directives_start .= '\|' . join(keys(g:antlers_custom_directives_pairs), '\|')
    let s:directives_end .= '\|' . join(values(g:antlers_custom_directives_pairs), '\|')
endif

setlocal autoindent
setlocal indentexpr=GetAntlersIndent()
exe 'setlocal indentkeys=o,O,<>>,!^F,0=}},0=!!},=@' . substitute(s:directives_end, '\\|', ',=@', 'g')

" Only define the function once.
if exists('*GetAntlersIndent')
    finish
endif
...
function! GetAntlersIndent()
    ...
    if exists('*GetAntlersIndentCustom')
        let indent = GetAntlersIndentCustom()
    elseif line !~# '^\s*\%(#\|//\)\|\*/\s*$' && (
